--------------------------------------------------------------------------------
-- Konstanten (wie in deiner Originaldatei)
--------------------------------------------------------------------------------
NET_PORT_FABRIC_REGISTRY      = 11
NET_NAME_FABRIC_REGISTRY_CLIENT = "FabricRegistryClient"
NET_NAME_FABRIC_REGISTRY_SERVER = "FabricRegistryServer"

NET_CMD_REGISTER              = "FabricRegistry.registerFabric"
NET_CMD_REGISTER_ACK          = "FabricRegistry.registerFabricAck"
NET_CMD_GET_FABRIC_UPDATE     = "FabricRegistry.getFabricUpdate"
NET_CMD_UPDATE_FABRIC         = "FabricRegistry.updateFabric"
NET_CMD_RESET_FABRICREGISTRY  = "FabricRegistry.resetFabricRegistry"



