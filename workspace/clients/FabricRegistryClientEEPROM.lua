...
function MyClient:initNetBoot()
    ...
    self._listenerId = event.registerListener(f, function(ev, nic, fromId, port, cmd, ...)
        ...
        computer.log(2, ('Net-Boot: Received reset command asasasasasasasasas from "%s"'):format(fromId))
        computer.log(2, "Funk: ")
...
local func = function() computer.log(3, "clean up before reset") end

c = MyClient:new()
c:setResetHandler(func)
c:initNetBoot()
